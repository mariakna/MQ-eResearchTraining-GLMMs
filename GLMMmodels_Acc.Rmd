...
(Acc1 <- summarySEwithin(sum, measurevar = "Acc", 
                       withinvars = "LC", 
                       idvar = "Subj", na.rm = FALSE, conf.interval = .95))

# correct vs. incorrect responses per naming attempt:
sum2 <- data2 %>%
  arrange(Subj, NamingAttempt) %>%
  group_by(Subj, NamingAttempt) %>%
  summarise(Acc = mean(Accuracy))

(Acc2 <- summarySEwithin(sum2, measurevar = "Acc", 
                       withinvars = "NamingAttempt", 
                       idvar = "Subj", na.rm = FALSE, conf.interval = .95))

# correct vs. incorrect responses per naming attempt per LC
sum3 <- data2 %>%
  arrange(Subj, NamingAttempt, LC) %>%
  group_by(Subj, NamingAttempt, LC) %>%
  summarise(Acc = mean(Accuracy))


(Acc3 <- summarySEwithin(sum3, measurevar = "Acc", 
...
(plot_acc <- ggplot(Acc3, aes(as.numeric(NamingAttempt), Acc, color = factor(LC))) +
  geom_line(size = 1) +
  geom_pointrange(aes(ymin = Acc - se, ymax = Acc + se), size = .4) +
