---
title: "Analysing RT data with (generalised) linear mixed effects models in R"
...
No wonder (remember the conditional $R^2$, for example!). The next step should probably be trying out the inverse link. One could also examine other distributions (e.g., Inverse Gaussian). We chose Gamma with the identity link based on the theoretical considerations; yet, in this case it seems like we might want to check out other options. Try this out in your own time and let me know what you get.
