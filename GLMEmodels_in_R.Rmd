...
2. Manipulations that slow RTs tend to increase both the mean and SD with relatively small effects in the skewed tail
...
In that case, we will fit a generalised linear mixed effects model with a gamma distribution and identity link.
...
[Barr et al., 2013](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3881361/) have recommended to always include the maximal random effects structure in the model, and some researchers follow this recommendation. However, this often leads to overparametrisation, with the resulting models being too complex and uninterpretable due to the inability of the model to provide accurate estimates (mostly because there is simply not enough data).

An alternative approach is, therefore, to fit a model that **is supported by the data** [(Bates et al., 2015)](https://arxiv.org/abs/1506.04967).
