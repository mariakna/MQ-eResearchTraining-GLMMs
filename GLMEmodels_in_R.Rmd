...
>>>>>>> a6d9a3ddecf1d4ff3ab161e5a32171a192feeace

## Option 2 (raw RTs): sneak peek 

We will fit a generalised linear mixed effects model with a gamma distribution and identity link.
...
```{r}
mod4_raw <- glmer(RT ~ Bas.SemUnRel + SemRel.SemUnRel +
                (1 + SemRel.SemUnRel||Subj) + 
                (1 + SemRel.SemUnRel||TargetAnswer), dataCorr3, 
                family = Gamma(link = "identity"))

summary(rePCA(mod4_raw))
VarCorr(mod4_raw)
```

Use all optimisers and compare the results:

```{r}
modelfit.all <- lme4::allFit(mod4_raw)
ss <- summary(modelfit.all)
```

```{r}
mod5_raw <- glmer(RT ~ Bas.SemUnRel + SemRel.SemUnRel +
                (1 + SemRel.SemUnRel||Subj) + 
                (1 + SemRel.SemUnRel||TargetAnswer), dataCorr3, 
                family = Gamma(link = "identity"),
                control = glmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 2e5)))

print(summary(mod5_raw, corr = F))
```

# cite https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#model-specification