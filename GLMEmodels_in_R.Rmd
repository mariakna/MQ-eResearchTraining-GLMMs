...
## Option 2: Do not transform TO BE CHANGED LATER
...
We now check to what extent the model is overfitting this specific dataset. To achieve this, we use bootsrap by randomly drawing observations from the data (with replacement). 

```{r}
length(unique(sample(1:4028, replace = T)))
```

Next, we fit a model to the data in the sample and use this model to predict the RTs in the original dataset (which contains data points on which the bootsrap sample hasn't been trained). Finally, we compare the goodness of fit of the two models. Averaged over the bootstrap samples, these comparisons can help us understand to what extent the original model overfits the data.
